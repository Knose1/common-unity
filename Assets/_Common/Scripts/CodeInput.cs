/*
	Code from https://github.com/Unity-Technologies/uGUI/blob/2019.1/UnityEngine.UI/UI/Core/InputField.cs

	Edit from the code :
	Added the code column (UI.Text) and the mouse scrolling
 */
...
namespace Com.Github.Knose1.Common.UI
{
		...
		IScrollHandler
	{
		...
		protected Text m_TextNumComponent;
		...
		/// </summary>
		public int caretWidth { get { return m_CaretWidth; } set { if (CustomSetPropertyUtility.SetStruct(ref m_CaretWidth, value)) MarkGeometryAsDirty(); } }
		...
		public Text textComponent
		{
			...
			set
			{
				...
				if (CustomSetPropertyUtility.SetClass(ref m_TextComponent, value))
				{
					EnforceTextHOverflow();
					if (m_TextComponent != null)
					{
		...
		/// </summary>
		public Color caretColor { get { return customCaretColor ? m_CaretColor : textComponent.color; } set { if (CustomSetPropertyUtility.SetColor(ref m_CaretColor, value)) MarkGeometryAsDirty(); } }
		...
		/// </code>
		/// </example>
		public Color selectionColor { get { return m_SelectionColor; } set { if (CustomSetPropertyUtility.SetColor(ref m_SelectionColor, value)) MarkGeometryAsDirty(); } }
		...
		/// </code>
		/// </example>
		public SubmitEvent onEndEdit { get { return m_OnEndEdit; } set { CustomSetPropertyUtility.SetClass(ref m_OnEndEdit, value); } }
		...
		/// </code>
		/// </example>
		public OnChangeEvent onValueChanged { get { return m_OnValueChanged; } set { CustomSetPropertyUtility.SetClass(ref m_OnValueChanged, value); } }
		...
		public int characterLimit
		{
			...
			set
			{
				if (CustomSetPropertyUtility.SetStruct(ref m_CharacterLimit, Math.Max(0, value)))
				{
					UpdateLabel();
					if (m_Keyboard != null)
		...
		public LineType lineType
		{
			...
			set
			{
				if (CustomSetPropertyUtility.SetStruct(ref m_LineType, value))
				{
		...
		public TouchScreenKeyboardType keyboardType
		{
			...
			set
			{
				if (CustomSetPropertyUtility.SetStruct(ref m_KeyboardType, value))
					SetToCustom();
			}
		}
		...
		/// </code>
		/// </example>
		public char asteriskChar { get { return m_AsteriskChar; } set { if (CustomSetPropertyUtility.SetStruct(ref m_AsteriskChar, value)) UpdateLabel(); } }
		...
		protected void UpdateLabel()
		{
			if (m_TextComponent != null && m_TextComponent.font != null && !m_PreventFontCallback)
			{
				...
				//Knose1
				string processedStart = processed;
				//End Knose1
				...
				if (!isEmpty)
				{
					...
					processedStart = processed.Substring(0, m_DrawStart);
				...
				//Knose1
				Regex findReturn = new Regex("\\n");

				int minIndex = findReturn.Matches(processedStart).Count;
				int count = findReturn.Matches(processed).Count;

				m_TextNumComponent.text = string.Join("\n", Enumerable.Range(minIndex + 1, count + 1));
				//End Knose1
		...
		/// </summary>
		/// <param name="eventData"></param>
		/// <author>Knose1</author>
		public void OnScroll(PointerEventData eventData)
		{
			/*Vector2 scroll = eventData.scrollDelta * scrollSpeed;
			int signY = (int)Mathf.Sign(scroll.y);


			if (signY != 0)
			{
				bool signYPositive = signY >= 1;
				MoveLeft()
			}*/
		}
...
{
	public static class CustomSetPropertyUtility
	{
